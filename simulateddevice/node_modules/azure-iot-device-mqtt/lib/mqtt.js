...
var Mqtt = /** @class */ (function (_super) {
    ...
    function Mqtt(config, provider) {
        ...
        _this._topicTelemetryPublish = 'devices/' + _this._config.deviceId + '/messages/events/';
        _this._topicMessageSubscribe = 'devices/' + _this._config.deviceId + '/messages/devicebound/#';
        _this._topicMethodSucbscribe = '$iothub/methods/POST/#';
        debug('topic publish: ' + _this._topicTelemetryPublish);
        debug('topic subscribe: ' + _this._topicMessageSubscribe);
        ...
        _this._topics = {
            'message': {
                name: _this._topicMessageSubscribe,
                listenersCount: 0,
            ...
            'method': {
                name: _this._topicMethodSucbscribe,
                listenersCount: 0,
                subscribeInProgress: false,
                subscribed: false,
        ...
        _this.on('newListener', function (eventName) {
            ...
            var topic = eventName.indexOf('method_') === 0 ?
                _this._topics.method :
                eventName === 'message' ?
                    _this._topics.message :
                    null;
            if (!!topic) {
                ...
                if (topic.subscribed === false && topic.subscribeInProgress === false) {
                    ...
                    _this._setupSubscription(topic);
                }
            }
        });
        _this.on('removeListener', function (eventName) {
            ...
            if (!!topic) {
                ...
                if (topic.listenersCount === 0 && topic.subscribed === true) {
                    ...
                    _this._removeSubscription(topic);
                }
            }
        });
            ...
            states: {
                ...
                connecting: {
                    _onEnter: function (connectCallback) {
                        _this._mqtt.connect(_this._config, function (err, result) {
                            debug('connect');
                            if (err) {
                                ...
                                /* Codes_SRS_NODE_DEVICE_MQTT_18_026: When MqttTransport fires the close event, the Mqtt object shall emit a disconnect event */
                                _this._mqtt.on('close', function (err) {
                                    _this._fsm.transition('disconnected', null, err);
                                });
    ...
    Mqtt.prototype.sendMethodResponse = function (response, done) {
        ...
        this._fsm.handle('sendMethodResponse', response, function (err) {
            if (done) {
                done(err);
            }
        });
    ...
    Mqtt.prototype._setupSubscription = function (topic) {
    ...
    Mqtt.prototype._removeSubscription = function (topic) {
