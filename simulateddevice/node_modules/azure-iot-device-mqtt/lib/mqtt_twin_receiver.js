...
var MqttTwinReceiver = /** @class */ (function (_super) {
    ...
    function MqttTwinReceiver(client) {
        ...
        _this._mqtt.on('message', _this._boundMessageHandler);
        return _this;
    }
        ...
        if (eventName === MqttTwinReceiver.responseEvent) {
            if (events_1.EventEmitter.listenerCount(this, MqttTwinReceiver.responseEvent) === 0) {
                ...
                process.nextTick(function () {
                    self._mqtt.subscribe(responseTopic, { qos: 0 }, function (err, transportObject) {
                        if (err) {
            ...
            if (events_1.EventEmitter.listenerCount(this, MqttTwinReceiver.postEvent) === 0) {
                ...
                process.nextTick(function () {
                    self._mqtt.subscribe(postTopic, { qos: 0 }, function (err, transportObject) {
                        if (err) {
                            self._handleError(err);
                        }
                        else {
    ...
    MqttTwinReceiver.prototype._handleRemoveListener = function (eventName) {
        ...
        if (eventName === MqttTwinReceiver.responseEvent) {
            if (events_1.EventEmitter.listenerCount(this, MqttTwinReceiver.responseEvent) === 0) {
                ...
                self._mqtt.unsubscribe(responseTopic, function (err) {
                    if (err) {
                        self._handleError(err);
                    }
                });
            ...
            if (events_1.EventEmitter.listenerCount(this, MqttTwinReceiver.postEvent) === 0) {
                ...
                self._mqtt.unsubscribe(postTopic, function (err) {
                    if (err) {
                        self._handleError(err);
                    }
                });
    ...
    MqttTwinReceiver.prototype._handleError = function (err) {
        ...
        this.emit(MqttTwinReceiver.errorEvent, azure_iot_mqtt_base_1.translateError(err));
    };
